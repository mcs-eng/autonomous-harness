...
/// Subscription readings for the model sheet's own first row. Opening the
/// sheet reads the cached snapshot immediately and refreshes at most once per
/// minute. No polling runs on startup or on a tab change.
///
/// The desktop's twin of this file backs the same row in its pane-header
/// picker — one shape, so a percentage read on the phone and the one read on
/// the laptop cannot disagree. The seam under it differs by device and not
/// here: [UsageController] on a phone reads no local credentials at all, only
/// `usage_read` over the relay, and the reason is written on that class.
