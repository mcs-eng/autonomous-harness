...
class IndexAwareCircularBuffer<T extends IndexedItem> {
  ...
  /// Atomically replaces a same-length range.
  ///
  /// This is required when [replacement] contains items that already belong to
  /// this buffer. Repeated `operator []=` assignments temporarily leave the
  /// same item in two slots; overwriting the source slot then detaches the item
  /// that was just adopted into the destination slot.
  void replaceRange(int index, List<T> replacement) {
    ...
    for (var offset = 0; offset < replacement.length; offset++) {
      _dropChild(index + offset);
    }
    for (var offset = 0; offset < replacement.length; offset++) {
      _adoptChild(index + offset, replacement[offset]);
    }
  }
