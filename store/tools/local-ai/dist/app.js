...
const gib = bytes => ((bytes || 0) / 1024 ** 3).toFixed(1);
...
function appendMessage(text, user = false) {
  ...
  else el.innerHTML = `<span class="message-label">OLLAMA</span><p class="message-text">${escapeHTML(text)}</p>`;
...
async function refresh() {
  try {
    const response = await fetch('/api/state'); if (!response.ok) throw new Error('Could not reach the Ollama viewer');
    applyState(await response.json());
  } catch (error) { $('#connection-status').textContent = 'Ollama viewer disconnected'; $('#operator-status').textContent = 'Reconnect to send a request'; return false; }
  return true;
}
...
function selectModel(model) {
  ...
  $('#operator-status').textContent = model || 'Ollama harness';
...
function render() {
  ...
  if (!notice.hidden) {
    const affected = lastJob.activeModel || lastJob.model || lastJob.models?.join(', ') || 'Ollama';
    const explanation = /tensor.*size overflow/i.test(lastJob.message)
      ? "Ollama could not read this model’s weights. Other models can still run."
  ...
  $('#connection-status').textContent = runtime.online ? 'Ollama connected' : 'Ollama offline';
  ...
  if (signature !== modelSignature) {
    ...
    $('#operator-status').textContent = selectedModel || 'Ollama harness';
  }
  ...
  $('#export-benchmarks').hidden = benchmarks.length === 0;
}
function renderBenchmarks() {
  ...
  const bars = (key, max, unit) => latest.map(b => `<div class="chart-row"><span>${escapeHTML(b.model)}</span><div class="bar-track"><div class="bar-fill ${key === 'medianFirstTokenMs' ? 'latency' : ''}" style="width:${Math.max(0, b[key] / max * 100)}%"></div></div><span class="bar-value">${number(b[key], key === 'medianFirstTokenMs' ? 0 : 1)} <small>${unit}</small></span></div>`).join('');
  $('#performance-content').innerHTML = `<div class="chart-heading"><h3>Generation speed</h3><span>tokens/s · higher is faster</span></div><div class="chart-bars" role="group" aria-label="Measured median generation speed">${bars('medianTokensPerSecond', maxSpeed, '')}</div><div class="chart-heading second-chart"><h3>Time to first token</h3><span>milliseconds · lower is faster</span></div><div class="chart-bars" role="group" aria-label="Measured median time to first token">${bars('medianFirstTokenMs', maxLatency, '')}</div><p class="benchmark-meta">Latest benchmark per model · median of 3 warm runs<br>Same prompt · 2,048-token context · up to 128 generated tokens · seed 42<br>Thinking off when supported; actual mode is recorded per run.<br>Repeated prompts can use the prefix cache. Speed measures inference, not answer quality.</p><details class="benchmark-details"><summary>Samples and measurement details</summary>${latest.map(b => `<div class="sample-record"><strong>${escapeHTML(b.model)}</strong><span>${new Date(b.at).toLocaleString()} · Ollama ${escapeHTML(b.runtime.version)}</span><span>Speed: ${b.samples.map(s => number(s.tokensPerSecond)).join(' / ')} tokens/s</span><span>First token: ${b.samples.map(s => number(s.firstTokenMs, 0)).join(' / ')} ms</span><span>Tokens generated: ${b.samples.map(s => s.tokens).join(' / ')} · thinking: ${escapeHTML(String(b.samples[0].thinking ?? 'not reported'))} · allocation: ${gib(b.residentBytes)} GiB</span></div>`).join('')}</details>`;
}
function showJob(job, historical = false) {
  ...
  const label = job.type === 'chat' ? job.model : 'OLLAMA';
  ...
  if (job.status === 'succeeded' && !completed.has(job.id)) {
    ...
    if (!historical && ['deploy', 'chat', 'benchmark'].includes(job.type) && job.result?.model) selectModel(job.result.model);
...
function inspectModel(id) {
  ...
  $('#model-detail').innerHTML = `<p class="eyebrow">OLLAMA / MODEL DETAILS</p><h2 id="detail-title">${escapeHTML(model.name)}</h2><p class="secondary">${model.running ? 'Loaded and ready for inference' : 'Downloaded and available to load'}</p><div class="detail-grid"><div><span>Model parameters</span><strong>${escapeHTML(model.parameters || 'Unknown')}</strong></div><div><span>Quantization</span><strong>${escapeHTML(model.quantization || 'Unknown')}</strong></div><div><span>Size on disk</span><strong>${gib(model.size)} GiB</strong></div><div><span>Runtime allocation</span><strong>${resident ? `${gib(resident.size)} GiB` : 'Not loaded'}</strong></div><div><span>Context window</span><strong>${resident?.context_length?.toLocaleString() || 'Set when loaded'}</strong></div><div><span>Idle expiry</span><strong>${resident?.expires_at ? new Date(resident.expires_at).toLocaleTimeString() : '—'}</strong></div></div><p class="secondary">Ollama reports runtime allocation. On Apple Silicon, CPU and GPU share unified memory; this is not a separate physical VRAM pool.</p><div class="actions"><button class="primary" data-action="deploy" data-target="${escapeHTML(id)}">${model.running ? 'Keep ready for 15m' : 'Load model'}</button><button data-action="benchmark" data-target="${escapeHTML(id)}">Benchmark</button><button data-chat="${escapeHTML(id)}">Chat</button>${model.running ? `<button data-action="unload" data-target="${escapeHTML(id)}">Unload</button>` : ''}</div>`;
...
if (document.modelContext?.registerTool) {
  ...
  try {
    await register({ name: 'harness_inventory', title: 'Inspect local models', description: 'Read Ollama models, residency, recent benchmarks and job status from this viewer.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, untrustedContentHint: true }, async execute() { if (!await refresh()) throw new Error('The workspace service is disconnected.'); return { runtime: state.runtime, benchmarks: state.benchmarks, jobs: state.jobs.map(({ id, type, model, status, message }) => ({ id, type, model, status, message })) }; } });
    await register({ name: 'harness_start_action', title: 'Control a local model', description: 'Start one local Ollama action. Deploy downloads a missing model. Benchmark runs three warm measurements. Returns a job ID; inspect inventory to see completion.', inputSchema: { type: 'object', properties: { type: { type: 'string', enum: ['deploy', 'download', 'unload', 'benchmark'] }, model: { type: 'string', description: 'Exact local Ollama model name and tag.' } }, required: ['type', 'model'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: true }, async execute(input) { if (!input || !['deploy', 'download', 'unload', 'benchmark'].includes(input.type) || typeof input.model !== 'string') throw new Error('Provide an allowed action and model.'); return runAction(input.type, input.model); } });
