...
export async function createBrandViewer(workspace,{home=homedir()}={}){
  ...
  async function picked(raw){
    ...
    if(!actual.toLowerCase().endsWith('.forme.json'))throw new Error('Choose a saved .forme.json project.');
    const info=await stat(actual);if(!info.isFile()||info.size>36000000)throw new Error('Choose a project file smaller than 36 MB.');
  ...
  async function recent(){
    ...
    for(const folder of ['Downloads','Desktop','Documents']){
      ...
      for(const entry of list.slice(0,3000)){if(!entry.isFile()||entry.name.startsWith('.')||!entry.name.toLowerCase().endsWith('.forme.json'))continue;const file=join(path,entry.name),info=await stat(file).catch(()=>null);if(info&&info.size<=36000000)files.push({name:entry.name,path:file,folder:'~/'+folder,mtime:info.mtimeMs});}
  ...
  const server=createServer(async(req,res)=>{
    ...
    async function body(){let size=0;const chunks=[];for await(const chunk of req){size+=chunk.length;if(size>36000000)throw new Error('Request exceeds 36 MB.');chunks.push(chunk);}return JSON.parse(Buffer.concat(chunks));}
    try{
      ...
      if(path==='/api/project'&&req.method==='PUT'){
        ...
        try{
          ...
          await writeFile(join(history,new Date().toISOString().replace(/[:.]/g,'-')+'-'+randomUUID()+'.forme.json'),JSON.stringify(before));
          const target=join(root,'board/project.json'),temporary=target+'.'+randomUUID()+'.tmp';
          await writeFile(temporary,JSON.stringify(project,null,2)+'\n');await rename(temporary,target);await buildBrand(root);
      ...
      if(path==='/'){const html=await readFile(join(root,'board/index.html'));return send(200,req.method==='HEAD'?'':html,'text/html; charset=utf-8');}
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  const server=await createBrandViewer(process.env.HARNESS_WORKSPACE);server.listen(port,'127.0.0.1',()=>console.log(`[forme] http://127.0.0.1:${port}/`));
