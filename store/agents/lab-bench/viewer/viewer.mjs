...
import { readProject,buildBench } from '../template/tools/build.mjs';
import { validateProject,digest,validateTransition } from '../template/studio/project.mjs';
import {verifySources} from '../template/studio/measurements.mjs';
export async function createBenchViewer(workspace){
  ...
  const server=createServer(async(req,res)=>{
    ...
    async function body(){let size=0;const chunks=[];for await(const chunk of req){size+=chunk.length;if(size>5000000)throw new Error('Request exceeds 5 MB.');chunks.push(chunk);}return JSON.parse(Buffer.concat(chunks));}
    try{
      ...
      if(path==='/api/project'&&req.method==='PUT'){
        ...
        saving=true;
        try{
          const project=await verifySources(await body());delete project.revision;
          ...
          validateTransition(before,project);const built=await buildBench(root,{project});
          ...
          await writeFile(join(history,new Date().toISOString().replace(/[:.]/g,'-')+'-'+randomUUID()+'.signal.json'),JSON.stringify(before));
          const target=join(root,'bench/project.json'),temporary=target+'.'+randomUUID()+'.tmp';
          const source={...project};delete source.revision;
          await writeFile(temporary,JSON.stringify(source,null,2)+'\n');
          const preview=join(root,'bench/index.html'),previewTemporary=preview+'.'+randomUUID()+'.tmp';await writeFile(previewTemporary,built.html);
          await rename(temporary,target);await rename(previewTemporary,preview);
          await writeFile(join(root,'.harness/verdict.json'),JSON.stringify({spec:1,ready:false,artifact:'bench/index.html',summary:'Source saved; measurement, model and report review pending'}));
          return send(200,{project:await current()});
        }finally{saving=false;}
      }
      if(req.method!=='GET'&&req.method!=='HEAD')return send(405,{error:'Method not allowed.'});
      if(path==='/'){const html=await readFile(join(root,'bench/index.html'));return send(200,req.method==='HEAD'?'':html,'text/html; charset=utf-8');}
...
if(process.argv[1]&&await realpath(process.argv[1]).catch(()=>null)===fileURLToPath(import.meta.url)){
  ...
  const server=await createBenchViewer(process.env.HARNESS_WORKSPACE);server.listen(port,'127.0.0.1',()=>console.log(`[signal] http://127.0.0.1:${port}/`));
