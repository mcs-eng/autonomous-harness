// The Lab Bench harness scripts, run the way Harness runs them.
...
function tempDir() { const d = mkdtempSync(join(tmpdir(), 'bench-')); scratch.push(d); return d }
...
const seedScript = join(here, 'skills/bench/scripts/seed-verdict.sh')
...
test('setup pins the numerical and build tools and resolves a managed Node runtime', () => {
  const pkg=JSON.parse(readFileSync(join(here,'toolchain/package.json'),'utf8'));
  assert.deepEqual(pkg.dependencies,{'esbuild':'0.27.2','playwright-core':'1.63.0','ml-matrix':'6.15.0','jstat':'1.9.6'});
...
test('seed-verdict reflects whether a bench exists', () => {
  const ws = tempDir()
  run(manifest.workspace.init, { cwd: ws })
  assert.equal(spawnSync(seedScript, [ws], { cwd: ws, encoding: 'utf8' }).status, 0)
  assert.equal(JSON.parse(readFileSync(join(ws, '.harness/verdict.json'), 'utf8')).ready, false)
  mkdirSync(join(ws, 'bench'), { recursive: true })
  writeFileSync(join(ws, 'bench/index.html'), '<!doctype html>')
  assert.equal(spawnSync(seedScript, [ws], { cwd: ws, encoding: 'utf8' }).status, 0)
  assert.equal(JSON.parse(readFileSync(join(ws, '.harness/verdict.json'), 'utf8')).ready, false)
})
