...
if [ ! -d upstream/.git ]; then
  ...
  git -C upstream sparse-checkout set tools lib schemas pipeline_defs skills styles .agents scripts remotion-composer backlot
fi
...
echo 'Installing the pinned OpenMontage composer'
(cd upstream/remotion-composer && npm ci --no-audit --no-fund)
...
echo 'Preparing the local film renderer'
(cd upstream/remotion-composer && node node_modules/@remotion/cli/remotion-cli.js browser ensure)
