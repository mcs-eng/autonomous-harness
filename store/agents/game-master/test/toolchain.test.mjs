// The Game Master harness scripts, run the way Harness runs them.
...
function tempDir() { const d = mkdtempSync(join(tmpdir(), 'game-')); scratch.push(d); return d }
...
const seedScript = join(here, 'skills/game/scripts/seed-verdict.sh')
...
test('seed-verdict reflects whether a game exists', () => {
  const ws = tempDir()
  run(manifest.workspace.init, { cwd: ws })
  assert.equal(spawnSync(seedScript, [ws], { cwd: ws, encoding: 'utf8' }).status, 0)
  assert.equal(JSON.parse(readFileSync(join(ws, '.harness/verdict.json'), 'utf8')).ready, false)
  mkdirSync(join(ws, 'game'), { recursive: true })
  writeFileSync(join(ws, 'game/index.html'), '<!doctype html>')
  assert.equal(spawnSync(seedScript, [ws], { cwd: ws, encoding: 'utf8' }).status, 0)
  assert.equal(JSON.parse(readFileSync(join(ws, '.harness/verdict.json'), 'utf8')).ready, false)
})
