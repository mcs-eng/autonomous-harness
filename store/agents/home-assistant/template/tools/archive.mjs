...
export function zip(files) {
  if (!files.length || files.length > 256)
    throw new Error("Archive needs 1–256 files.");
  const chunks = [],
    central = [],
    names = new Set();
  let offset = 0,
  ...
  for (const { name, bytes: input } of files) {
    if (
      (name !== ".gitignore" && !/^[a-zA-Z0-9][a-zA-Z0-9._/-]*$/.test(name)) ||
    ...
    if (total > 128 * 1024 * 1024)
      throw new Error("Portable project exceeds 128 MiB uncompressed.");
  ...
  if (output.length > 32 * 1024 * 1024)
    throw new Error("Portable project exceeds the 32 MiB download limit.");
  return output;
}

// Read ZIP members in memory, never extract paths. Used for independently reopening portable projects.
export function unzip(input) {
  const data = Buffer.from(input);
  if (data.length > 32 * 1024 * 1024) throw new Error("ZIP exceeds 32 MiB.");
  let end = -1;
  for (let p = data.length - 22; p >= Math.max(0, data.length - 65557); p--)
    if (
      data.readUInt32LE(p) === 0x06054b50 &&
      p + 22 + data.readUInt16LE(p + 20) === data.length
    ) {
      end = p;
      break;
    }
  ...
  for (let i = 0; i < count; i++) {
    ...
    const flags = data.readUInt16LE(cursor + 8),
      ...
      length = data.readUInt32LE(cursor + 24),
      n = data.readUInt16LE(cursor + 28),
      extra = data.readUInt16LE(cursor + 30),
      comment = data.readUInt16LE(cursor + 32),
      offset = data.readUInt32LE(cursor + 42);
    if (
      flags & 1 ||
      ![0, 8].includes(method) ||
      ...
      throw new Error("Encrypted, split or ZIP64 files are unsupported.");
    if (cursor + 46 + n + extra + comment > end)
      throw new Error("Truncated ZIP entry.");
    const name = data.subarray(cursor + 46, cursor + 46 + n).toString("utf8"),
    ...
    if (((data.readUInt32LE(cursor + 38) >>> 16) & 0xf000) === 0xa000)
      throw new Error("ZIP symlinks are unsupported.");
    seen.add(key);
    total += length;
    if (total > 128 * 1024 * 1024)
    ...
    const ln = data.readUInt16LE(offset + 26),
      le = data.readUInt16LE(offset + 28),
      body = offset + 30 + ln + le,
      finish = body + compressed;
    if (
      finish > start ||
      !data.subarray(offset + 30, offset + 30 + ln).equals(Buffer.from(name))
    )
      throw new Error("ZIP member bounds/name mismatch.");
    if (ranges.some(([a, b]) => offset < b && finish > a))
      throw new Error("Overlapping ZIP members.");
    ranges.push([offset, finish]);
    const bytes =
      method === 0
        ? data.subarray(body, finish)
        : inflateRawSync(data.subarray(body, finish), {
            maxOutputLength: Math.max(1, length),
          });
    if (bytes.length !== length || crc32(bytes) !== crc)
      throw new Error("ZIP member checksum mismatch.");
