// The Generative Art harness scripts, run the way Harness runs them.
...
function tempDir() { const d = mkdtempSync(join(tmpdir(), 'genart-')); scratch.push(d); return d }
...
const seedScript = join(here, 'skills/gen-art/scripts/seed-verdict.sh')
...
test('setup installs package-local export tools and finds a browser', () => {
...
test('doctor verifies authoring and export tools', () => {
...
test('seed-verdict reflects whether a sketch exists', () => {
  const ws = tempDir()
