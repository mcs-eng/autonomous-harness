...
const allowed = /^(?:part\.(?:step|stl|FCStd)|handoff\/(?:index\.html|checks\.json|project\.zip|parts\/[A-Za-z][A-Za-z0-9_-]{0,63}\.(?:step|stl|svg)))$/;
const types = {'.html':'text/html; charset=utf-8', '.json':'application/json; charset=utf-8', '.svg':'image/svg+xml', '.zip':'application/zip', '.step':'model/step', '.stl':'model/stl', '.FCStd':'application/octet-stream'};
...
export async function serve(workspace, port=0) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (!['.html','.svg'].includes(extension)) headers['Content-Disposition'] = `attachment; filename="${name.split('/').at(-1)}"`;
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(`Saved FreeCAD handoff: http://127.0.0.1:${server.address().port}/handoff/index.html`);
