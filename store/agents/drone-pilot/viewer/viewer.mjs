...
import { readProject,buildFlight } from '../template/tools/build.mjs';
import { validateProject } from '../template/studio/project.mjs';
const hash=p=>createHash('sha256').update(JSON.stringify(p)).digest('hex');
export async function createFlightViewer(workspace,{home=homedir()}={}){
  ...
  async function picked(raw){
    ...
    if(!actual.toLowerCase().endsWith('.vector.json'))throw new Error('Choose a saved .vector.json project.');
    const info=await stat(actual);if(!info.isFile()||info.size>8000000)throw new Error('Choose a project file smaller than 8 MB.');
  ...
  async function recent(){
    ...
    for(const folder of ['Downloads','Desktop','Documents']){
      ...
      for(const entry of list.slice(0,3000)){if(!entry.isFile()||entry.name.startsWith('.')||!entry.name.toLowerCase().endsWith('.vector.json'))continue;const file=join(path,entry.name),info=await stat(file).catch(()=>null);if(info&&info.size<=8000000)files.push({name:entry.name,path:file,folder:'~/'+folder,mtime:info.mtimeMs});}
  ...
  const server=createServer(async(req,res)=>{
    ...
    async function body(){let size=0;const chunks=[];for await(const chunk of req){size+=chunk.length;if(size>8000000)throw new Error('Request exceeds 8 MB.');chunks.push(chunk);}return JSON.parse(Buffer.concat(chunks));}
    try{
      ...
      if(path==='/api/project'&&req.method==='PUT'){
        ...
        try{
          ...
          const built=await buildFlight(root,{project});
          ...
          await writeFile(join(history,new Date().toISOString().replace(/[:.]/g,'-')+'-'+randomUUID()+'.vector.json'),JSON.stringify(before));
          const target=join(root,'flight/project.json'),temporary=target+'.'+randomUUID()+'.tmp';
          await writeFile(temporary,JSON.stringify(project,null,2)+'\n');
          const preview=join(root,'flight/index.html'),previewTemporary=preview+'.'+randomUUID()+'.tmp';await writeFile(previewTemporary,built.html);
          await rename(temporary,target);await rename(previewTemporary,preview);
          await writeFile(join(root,'.harness/verdict.json'),JSON.stringify({spec:1,ready:false,artifact:'flight/index.html',summary:'Source saved; survey and delivery review pending'}));
      ...
      if(path==='/'){const html=await readFile(join(root,'flight/index.html'));return send(200,req.method==='HEAD'?'':html,'text/html; charset=utf-8');}
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  const server=await createFlightViewer(process.env.HARNESS_WORKSPACE);server.listen(port,'127.0.0.1',()=>console.log(`[vector] http://127.0.0.1:${port}/`));
