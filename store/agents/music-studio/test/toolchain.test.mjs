// The Music Studio harness scripts, run the way Harness runs them.
...
function tempDir() { const d = mkdtempSync(join(tmpdir(), 'studio-')); scratch.push(d); return d }
...
const seedScript = join(here, 'skills/studio/scripts/seed-verdict.sh')
...
test('seed-verdict reflects whether a piece exists', () => {
  const ws = tempDir()
  run(manifest.workspace.init, { cwd: ws })
  assert.equal(spawnSync(seedScript, [ws], { cwd: ws, encoding: 'utf8' }).status, 0)
  assert.equal(JSON.parse(readFileSync(join(ws, '.harness/verdict.json'), 'utf8')).ready, false)
  mkdirSync(join(ws, 'piece'), { recursive: true })
  writeFileSync(join(ws, 'piece/index.html'), '<!doctype html>')
  assert.equal(spawnSync(seedScript, [ws], { cwd: ws, encoding: 'utf8' }).status, 0)
  assert.equal(JSON.parse(readFileSync(join(ws, '.harness/verdict.json'), 'utf8')).ready, false)
})
