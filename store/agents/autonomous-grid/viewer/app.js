(() => {
  ...
  function renderInspector() {
    ...
    if(node){
      ...
      const hostGpu=node.gpuTelemetry?.scope==='shared-host',gpuLabel=hostGpu?'Host GPU':'GPU';
      const gpuState=hostGpu?`<div class="detail-section"><h3>Host GPU · shared workloads</h3>${pair('Device',node.gpuTelemetry.name||'—')}${pair('Sensor observation',node.gpuTelemetry.observedAt?age(node.gpuTelemetry.observedAt):'Unavailable')}${node.gpuTelemetry.error?`<div class="source-error">${escape(node.gpuTelemetry.error)}</div>`:''}</div>`:'';
      panel.innerHTML=`<div class="inspector-head"><div><span class="eyebrow">ENGINE DETAILS</span><h2>${escape(node.name)}</h2><p class="subline">${escape(node.hardware || node.engine)}${node.platform?`<br>${escape(node.platform)}`:''}</p></div><button class="close" type="button" data-close aria-label="Close engine details">×</button></div><div class="detail-status"><span class="status-dot${node.stale?' pending':node.online===false?' offline':''}"></span>${node.stale?'Last known observation':node.online===true?'Online':node.online===false?'Offline':'Status not reported'}${node.hosted?' · hosted model':''}</div><div class="detail-metrics">${detailMetric('Last decode rate',node.tokS,'tok/s','Per-engine estimate')}${detailMetric(`${gpuLabel} temperature`,node.temperatureC,'°C')}${detailMetric(`${gpuLabel} utilization`,node.utilizationPct,'%')}${detailMetric(`${gpuLabel} power`,node.powerW,'W',node.powerLimitW!==null?`${fmt(node.powerLimitW)} W limit`:'')}</div><div class="detail-section"><h3>${escape(node.memoryKind || 'Model memory')}${hostGpu?' · shared workloads':''}${node.stale?' · last known':''}</h3>${node.hosted&&!hostGpu?'<p class="quiet">Hosted models do not contribute local GPU memory.</p>':`<div class="detail-pair"><strong>${fmt(used)} <span>/ ${fmt(total)} GB</span></strong></div><div class="memory-track"><span style="width:${fill}%"></span></div><div class="memory-labels"><span>${fmt(node.memoryFreeGb)} GB free</span><span>${total>0&&finite(used)?fmt(used/total*100,0)+'% used':'not reported'}</span></div>`}</div>${gpuState}<div class="detail-section"><h3>${escape(hostGpu&&metric!=='tokS'?`Host ${metricInfo[metric][0]}`:metricInfo[metric][0])} · recent observations</h3>${spark(node.id)}</div><div class="detail-section"><h3>Models on this engine · ${node.models.length}</h3>${node.models.map(model=>{const caps=node.capabilities?.find(c=>c.model===model);return `<div class="model-entry"><span class="model-glyph">◇</span><span class="model-title">${escape(model)}<span class="model-caption">${caps?.contextLength?`${fmt(caps.contextLength,0)} context tokens`:''}${caps?.responses?' · Responses API':''}</span></span></div>`;}).join('')||'<p class="quiet">No models advertised.</p>'}</div><div class="detail-section">${pair('Engine',node.engine)}${pair('Concurrency',fmt(node.concurrency,0))}${pair('Active requests',fmt(node.activeRequests,0))}${pair('Disk',`${fmt(node.diskUsedGb)} / ${fmt(node.diskTotalGb)} GB`)}${pair('Output tokens',fmt(node.answered?.tokensOut,0))}${pair('Usage window',node.answered?.windowSeconds?`${fmt(node.answered.windowSeconds/3600)} hours`:'—')}${pair('Last observed',age(node.observedAt))}${node.endpoint?`<div class="endpoint">${escape(node.endpoint)}</div>`:''}</div>${prompt(`Review the models on ${node.name} and suggest a better placement for my workload, using measured memory and performance.`)}`;
      return;
    }
