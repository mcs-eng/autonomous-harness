...
function appendMessage(text, user = false) {
  ...
  else el.innerHTML = `<span class="message-label">MLX-LM</span><p class="message-text">${escapeHTML(text)}</p>`;
...
async function refresh() {
  try {
    const response = await fetch('/api/state'); if (!response.ok) throw new Error('Could not reach the MLX-LM viewer');
    applyState(await response.json());
  } catch (error) { $('#connection-status').textContent = 'MLX-LM viewer disconnected'; $('#operator-status').textContent = 'Reconnect to send a request'; return false; }
  return true;
}
...
function selectModel(model) {
  ...
  $('#operator-status').textContent = model || 'MLX-LM harness';
...
function render() {
  ...
  if (!notice.hidden) {
    const affected = lastJob.activeModel || lastJob.model || lastJob.models?.join(', ') || 'MLX-LM';
    const explanation = /tensor.*size overflow/i.test(lastJob.message)
      ? "MLX-LM could not read this model’s weights. Other models can still run."
  ...
  $('#connection-status').textContent = runtime.online ? 'MLX-LM connected' : 'MLX-LM offline';
  ...
  if (signature !== modelSignature) {
    ...
    $('#operator-status').textContent = selectedModel || 'MLX-LM harness';
  }
...
function showJob(job, historical = false) {
  ...
  const label = job.type === 'chat' ? job.model : 'MLX-LM';
...
if (document.modelContext?.registerTool) {
  ...
  try {
    await register({ name: 'harness_inventory', title: 'Inspect local models', description: 'Read MLX-LM models, residency, recent benchmarks and job status from this viewer.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, untrustedContentHint: true }, async execute() { if (!await refresh()) throw new Error('The workspace service is disconnected.'); return { runtime: state.runtime, benchmarks: state.benchmarks, jobs: state.jobs.map(({ id, type, model, status, message }) => ({ id, type, model, status, message })) }; } });
    await register({ name: 'harness_start_action', title: 'Control a local model', description: 'Start one local MLX-LM action. Deploy downloads a missing model. Benchmark runs three warm measurements. Returns a job ID; inspect inventory to see completion.', inputSchema: { type: 'object', properties: { type: { type: 'string', enum: ['deploy', 'download', 'unload', 'benchmark'] }, model: { type: 'string', description: 'Exact Hugging Face MLX model ID, including its owner.' } }, required: ['type', 'model'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: true }, async execute(input) { if (!input || !['deploy', 'download', 'unload', 'benchmark'].includes(input.type) || typeof input.model !== 'string') throw new Error('Provide an allowed action and model.'); return runAction(input.type, input.model); } });
